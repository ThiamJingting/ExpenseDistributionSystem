...
$queryMembers = "SELECT * from group_member_kr WHERE Group_name = 'Group A'";
$resultMembers = mysqli_query($link, $queryMembers) or die(mysqli_error($link));

$queryMembers2 = "SELECT * from group_member_kr WHERE Group_name = 'Group A'";
$resultMembers2 = mysqli_query($link, $queryMembers2) or die(mysqli_error($link));
$queryMembers3 = "SELECT * from group_member_kr WHERE Group_name = 'Group A'";
$resultMembers3 = mysqli_query($link, $queryMembers3) or die(mysqli_error($link));

$queryId = "SELECT DISTINCT Exp_ID FROM expense_table_kr;";
$resultId = mysqli_query($link, $queryId)or die(mysqli_error($link));

$lastIDquery = "SELECT * FROM expense_table_kr ORDER BY Exp_ID desc limit 1";
            ...
            $(document).ready(function () {
                ...
                $("select#currency").change(function () {
                    ...
                    if (currency == "JPY") {
                        ...
                        $("input#exchange_rate").val("0.0445");
                        var rate = 0.0445;
                        var sAmount = $("input#totalAmt").val();
                        if (sAmount != "") {
                            var amt = parseFloat(sAmount);
                ...
                function checkCurrencyImg(curr, memberId) {
                    ...
                    if (curr == "JPY") {
                        ...
                        $('#curr').attr('src', 'img/TWD.png');
                        $('#curr' + memberId).attr('src', 'img/TWD.png');
                    } else if (curr == "SGD") {
                        $('#curr').attr('src', 'img/SGD.png');
        ...
        <script>
            ...
            $("#addMember").click(function () {
                ...
                if (descr == "" || id == "0" || date == "" || type == "0" || portion1 == "" || mem1 == "0" || pay1 == "") {
                    ...
                    $("select#m4 option[value='" + mem1 + "']").remove();
                    $("select#m4 option[value='" + mem2 + "']").remove();
                    $("select#m4 option[value='" + mem3 + "']").remove();
                }
            });
